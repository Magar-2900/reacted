	...
	class CelebrityModel extends CI_Model 
	{
		...
		public function get_special_celebrities(){
			$this->db->select("users.iUsersId as user_id,users.vFirstName as first_name,users.vLastName as last_name,users.vEmail as email,users.vPhone as phone,user_roles.vRole as role,user_celebrity.vTitle as title,user_celebrity.vTagLine as tag_line,user_celebrity.vShortDescription as short_description,user_celebrity.vLongDescription as long_description,user_celebrity.dPrice as price,user_celebrity.eIsFeatured as is_featured,user_celebrity.dtAddedDate as added_date,user_celebrity.dtUpdatedDate as updated_date,GROUP_CONCAT(category_master.vCategoryName SEPARATOR ',') as categories,users.vImage as images,vW9Form as w9form,vSocialMediaLinks as social_media_links, special_celebrities.special_start_date as special_start_date, special_celebrities.special_end_date as special_end_date, special_celebrities.total_off as total_off");
			$this->db->from('users');
			$this->db->join('user_roles','user_roles.iRoleId = users.iRoleId','left');
			$this->db->join('user_celebrity','user_celebrity.iUsersId = users.iUsersId','left');
			$this->db->join("category_master","find_in_set(category_master.iCategoryMasterId,user_celebrity.vCategories)<> 0","left",false);
